...
class PowerRankings{
  public:
    ...
    //Mutator method to add newly created Sim with default values to both maps
...
void PowerRankings::decrease(string name, int points){
  ...
  if(names[name].second < ranks.size()){
    //trailing is the Sim currently ranked below the Sim having their points decreased
...
void PowerRankings::addRank(string name){
  ...
  pair <int, int> simStats = make_pair(0, names.size()+1);
  names[name] = simStats;
...
void PowerRankings::printRanks(void){
  ...
  for(auto it = ranks.begin(); it != ranks.end(); ++it){
    cout << "Rank " << it->first << " is " << it->second << ": " << names[it->second].first << " Points " << "\n";
  };
  cout << "\n";
}
