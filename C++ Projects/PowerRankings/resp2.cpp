...
void PowerRankings::increase(string name, int points){
  ...
  if(names[name].second > 1){
    ...
    while(names[leading].first < names[name].first){
      ...
      leading = ranks[names[name].second-1]; // Update leading to the new Sim at the rank above
    }
  }
}
...
void PowerRankings::decrease(string name, int points){
  ...
  if(names[name].second < ranks.size()){
    //trailing is the Sim currently ranked below the Sim having their points lowered
    string trailing = ranks[names[name].second+1];
    while(names[name].first < names[trailing].first){
      swap(names[name].second, names[name].second+1); // Pass the correct rank to the swap function
      if(names[name].second == ranks.size()){
        break;
      }
      trailing = ranks[names[name].second+1]; // Update trailing to the new Sim at the rank below
    }
  }
}
...
int main() {
  ...
  //increase someone's points
  cout << "\nincreasing Soju's points by 10.\n";
  if(TopSims.names.find("Soju") != TopSims.names.end()) {
    ...
    cout << "Soju does not exist.\n";
  }

  cout << "\nincreasing Mary's points by 15.\n";
  if(TopSims.names.find("Mary") != TopSims.names.end()) {
