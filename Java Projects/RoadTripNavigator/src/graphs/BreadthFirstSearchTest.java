...
class BreadthFirstSearchTest {
	...
	private static BreadthFirstSearch bfsearch;
	City[] Cities;
	...
	void setUp() throws Exception {
		...
		bfsearch = new BreadthFirstSearch(G, 0);
		
		infile.close();
		//Output the Path
		System.out.println("BFS: " + bfsearch.pathTo(21, Cities));
	}

	@Test
	void test() throws FileNotFoundException {
		//Call the function that check if all paths are checked. It's value should be true
		assertTrue(bfsearch.allPathsChecked());
		...
		while(currentCity != bfsearch.start()) {	 //Until you get to the starting city, going from end of path to start
			...
			for (int i = 0; i < E; i++) {
				...
				if(cityNames[0].equalsIgnoreCase(Cities[bfsearch.edgeTo(currentCity)].CityName()) && cityNames[1].equalsIgnoreCase(Cities[currentCity].CityName()))
					validEdge = true;
			}
			...
			currentCity = bfsearch.edgeTo(currentCity);
		}
	}
